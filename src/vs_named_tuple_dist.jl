...
function _flat_ntdistelem_to_stdmv(trg::StandardDist{<:Any,1}, sd::Distribution, x_unshaped::AbstractVector{<:Real}, trg_acc::ValueAccessor)
    ...
    vartransform_def(td, unshaped(sd), sv)
end
...
function MeasureBase.vartransform_def(trg::StandardDist{<:Any,1}, src::ValueShapes.UnshapedNTD, x::AbstractVector{<:Real})
...
function MeasureBase.vartransform_def(trg::StandardDist{<:Any,1}, src::NamedTupleDist, x::Union{NamedTuple,ShapedAsNT})
    x_unshaped = unshaped(x, varshape(src))
    vartransform_def(trg, unshaped(src), x_unshaped)
end
...
function _stdmv_to_flat_ntdistelem(td::Distribution, src::StandardDist{<:Any,1}, x::AbstractVector{<:Real}, src_acc::ValueAccessor)
    ...
    sv = src_acc(x)
    vartransform_def(unshaped(td), sd, sv)
end
...
function MeasureBase.vartransform_def(trg::ValueShapes.UnshapedNTD, src::StandardDist{<:Any,1}, x::AbstractVector{<:Real})
    @argcheck length(src) == length(eachindex(x))
...
function MeasureBase.vartransform_def(trg::NamedTupleDist, src::StandardDist{<:Any,1}, x::AbstractVector{<:Real})
    unshaped_result = vartransform_def(unshaped(trg), src, x)

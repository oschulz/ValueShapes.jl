...
@testset "NamedTupleDist" begin
    @testset "NamedTupleDist functionality" begin
    ...
    @testset "NamedTupleDist transport" begin
        ...
        test_transport(dist2, dist1)
        test_transport(dist1, dist2)

        for VT in (NamedTuple, ShapedAsNT)
            μ_dist = unshaped(NamedTupleDist(VT, a = Weibull(), b = MvNormal([1.3 0.6; 0.6 2.4])))
            f = transport_to(Normal, μ_dist)
            x = rand(μ_dist)
